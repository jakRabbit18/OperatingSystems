...
int main(int argc, char **argv) {
	...
	for(std::vector<MAILFLAG>::iterator it = mflags.begin(); it < mflags.end(); it++) {
		if(it == mflags.begin()) {
			//this is thread 0, make it the main thread
			if (pthread_create(&(it->thread), NULL, mainThread, (void *)&numThreads) != 0) {
	...
	while (scanf("%d", &k) == 1) {
		if(count++ % 2) {
			...
			if(toThread <= numThreads)
				sendMessage(toThread, m);
			else 
				printf("No such thread: %d\n", toThread);
		} else {
			// printf("read number: %d", k);
			mVal = k;
		}
	}
